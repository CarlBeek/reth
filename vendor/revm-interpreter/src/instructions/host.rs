...
pub fn balance<WIRE: InterpreterTypes, H: Host + ?Sized>(context: InstructionContext<'_, H, WIRE>) {
    ...
    if spec_id.is_enabled_in(BERLIN) {
        ...
        *top = account.balance;
        ...
        let Ok(account) = context.host.load_account_info_skip_cold_load(address, false, false)
        else {
...
pub fn extcodesize<WIRE: InterpreterTypes, H: Host + ?Sized>(
    ...
    if spec_id.is_enabled_in(BERLIN) {
        ...
        *top = U256::from(account.code.as_ref().unwrap().len());
        ...
        let Ok(account) = context.host.load_account_info_skip_cold_load(address, true, false)
        else {
            return context.interpreter.halt_fatal();
        };
        // safe to unwrap because we are loading code
...
pub fn extcodecopy<WIRE: InterpreterTypes, H: Host + ?Sized>(
    ...
    if len != 0 {
        ...
        resize_memory!(context.interpreter, context.host.gas_params(), memory_offset_usize, len);
    }
...
pub fn sstore<WIRE: InterpreterTypes, H: Host + ?Sized>(context: InstructionContext<'_, H, WIRE>) {
    ...
    // If gasleft is less than or equal to gas stipend, fail the current call frame with ‘out of
    // gas’ exception.
    ...
    let state_load = if spec_id.is_enabled_in(BERLIN) {
        let additional_cold_cost = context.host.gas_params().cold_storage_additional_cost();
        let skip_cold = context.interpreter.gas.remaining() < additional_cold_cost;
        let res = context.host.sstore_skip_cold_load(target, index, value, skip_cold);
        match res {
            Ok(load) => load,
    ...
    // refund
    let refund = if is_eip8037 {
        let state_refund = context.host.gas_params().sstore_state_refund(&state_load.data);
        if state_refund > 0 {
            context.interpreter.gas.refill_state_gas(state_refund);
        }
        context.host.gas_params().sstore_refund_after_state_refill(is_istanbul, &state_load.data)
    } else {
        context.host.gas_params().sstore_refund(is_istanbul, &state_load.data)
    };
