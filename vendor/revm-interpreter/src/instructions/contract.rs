...
pub fn create<WIRE: InterpreterTypes, const IS_CREATE2: bool, H: Host + ?Sized>(
    ...
    if len != 0 {
        ...
        resize_memory!(context.interpreter, context.host.gas_params(), code_offset, len);
    ...
    let mut refundable_state_gas = 0;
    if context.host.is_amsterdam_eip8037_enabled() {
        refundable_state_gas = context.host.gas_params().create_state_gas();
        state_gas!(context.interpreter, refundable_state_gas);
    }
    ...
    create_inputs.set_refundable_state_gas(refundable_state_gas);
    context
        .interpreter
        .bytecode
...
pub fn call<WIRE: InterpreterTypes, H: Host + ?Sized>(
    ...
    let Some((gas_limit, refundable_state_gas, bytecode, bytecode_hash)) =
