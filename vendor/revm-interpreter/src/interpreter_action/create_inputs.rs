...
impl CreateInputs {
    ...
    pub fn created_address(&self, nonce: u64) -> Address {
        *self.cached_address.get_or_init(|| match self.scheme {
            ...
            CreateScheme::Create2 { salt } => {
                self.caller.create2_from_code(salt.to_be_bytes(), &self.init_code)
            }
    ...
    /// Returns parent-charged state gas to refill if this frame fails.
    pub fn refundable_state_gas(&self) -> u64 {
        self.refundable_state_gas
    }

    /// Sets parent-charged state gas to refill if this frame fails.
    pub fn set_refundable_state_gas(&mut self, refundable_state_gas: u64) {
        self.refundable_state_gas = refundable_state_gas;
    }
}
