...
///
/// As CallInput uses shared memory buffer it can get overridden if not used directly when call
/// happens.
...
pub enum CallInput {
    ...
    Bytes(Bytes),
    /// The Range points to the SharedMemory buffer. Buffer can be found in
    /// [`context_interface::LocalContextTr::shared_memory_buffer_slice`] function. And can be
    /// accessed with `evm.ctx().local().shared_memory_buffer()`
    ///
    /// # Warning
    ///
    /// Use it with caution, CallInput shared buffer can be overridden if context from child call
    /// is returned so recommendation is to fetch buffer at first Inspector call and clone it
    /// from [`context_interface::LocalContextTr::shared_memory_buffer_slice`] function.
...
impl CallInput {
    ...
    /// shared memory context to its parent, the range in `CallInput::SharedBuffer` can show
    /// unexpected data.
    ///
    /// # Allocation
    ///
...
pub struct CallInputs {
    ...
    pub reservoir: u64,
    /// State gas charged in the parent before this frame and refilled if this frame fails.
