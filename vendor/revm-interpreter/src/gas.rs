...
/// **State gas charges** (`record_state_cost`): deduct from `reservoir` first; when exhausted,
/// spill into `remaining`. Total gas available = `remaining` + `reservoir`.
///
...
impl Gas {
    ...
    /// Creates a new `Gas` struct with a regular gas budget and reservoir (EIP-8037 reservoir
    /// model).
    ///
    ...
    pub const fn new_spent(limit: u64) -> Self {
        Self { tracker: GasTracker::new(limit, 0, 0), memory: MemoryGas::new() }
    }
    ...
    pub fn refill_state_gas(&mut self, amount: u64) {
        self.tracker.refill_state_gas(amount);
    }
...
mod tests {
    ...
    #[test]
    fn test_refill_state_gas_restores_reservoir() {
        let mut gas = Gas::new_with_regular_gas_and_reservoir(500, 100);

        assert!(gas.record_state_cost(250));
        assert_eq!((gas.reservoir(), gas.remaining(), gas.state_gas_spent()), (0, 350, 250));

        gas.refill_state_gas(250);
        assert_eq!((gas.reservoir(), gas.remaining(), gas.state_gas_spent()), (250, 350, 0));
    }
}
