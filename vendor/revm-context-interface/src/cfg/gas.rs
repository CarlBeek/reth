...
pub struct GasTracker {
    ...
    /// and is reset by [`Self::rollback_state_gas`]), this increments on every
    /// [`Self::record_state_cost`] call and is never rolled back, so it
    /// survives an out-of-gas at a state-creating op. Lets downstream tooling
    /// report "this op needed N state gas" instead of a misleading 0 when the
    /// op OOG'd. Not a consensus value — diagnostic only.
...
impl GasTracker {
    ...
    /// only; see the field docs on [`GasTracker`].
    #[inline]
    ...
    #[inline]
    pub const fn set_state_gas_demanded(&mut self, val: u64) {
    ...
    pub const fn record_state_cost(&mut self, cost: u64) -> bool {
        ...
        // does not affect the affordability decisions below.
