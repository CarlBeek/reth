...
pub const fn handle_reservoir_remaining_gas(
    ...
    // regardless of success/revert/halt, so the top-level tracker reflects what
    // every frame *attempted* — including a frame that OOG'd at a state op and
    // whose state_gas_spent was therefore rolled back to 0 below. Child frames
    // start demanded at 0, so adding (not overwriting) preserves the parent's
    // own prior attempts. Read before the child settles, since rollback does
    // not touch this counter.
    parent_gas.set_state_gas_demanded(
        parent_gas.state_gas_demanded().saturating_add(child_gas.state_gas_demanded()),
    );
