...
pub trait Handler {
    ...
    fn runtime_oog_result(
        ...
        // reservoir.
        // Diagnostic (EIP-8037): the runtime phase's charges are dropped by the
        // rebuild below, but the *attempted* tally must survive it — this is
        // precisely the case the counter exists for, a tx that OOG'd in the
        // runtime state-gas phase (auth list or first frame) and would
        // otherwise report 0 state gas needed.
