...
//!
//! These EIPs require the client to perform special system calls to update state (such as block
//! hashes or beacon roots) at block boundaries, outside of normal EVM transaction execution. REVM
//! provides the system call mechanism, but the actual state transitions must be performed by the
//! client or test harness, not by the EVM itself.
//!
//! # Example: Using `system_call` for pre/post block hooks
//!
//! The client should use [`SystemCallEvm::system_call`] method to perform required state updates
//! before or after block execution, as specified by the EIP:
//!
//! ```rust,ignore
...
//! ```
//!
//! See the book section on [External State
//! Transitions](../../book/src/external_state_transitions.md) for more details.
use crate::{
    frame::EthFrame, instructions::InstructionProvider, EvmTr, ExecuteCommitEvm, ExecuteEvm,
...
/// Regular gas budget for system calls.
pub const SYSTEM_CALL_REGULAR_GAS_LIMIT: u64 = 30_000_000;
/// Maximum number of new storage slots one system call is expected to write.
pub const SYSTEM_MAX_SSTORES_PER_CALL: u64 = 16;

/// Returns the EIP-8037 system-call state-gas reservoir.
#[inline]
pub fn system_call_state_gas_reservoir(cfg: impl Cfg) -> u64 {
    if cfg.is_amsterdam_eip8037_enabled() {
        cfg.gas_params().sstore_set_state_gas().saturating_mul(SYSTEM_MAX_SSTORES_PER_CALL)
    } else {
        0
    }
}

/// Returns the total gas limit for a system call transaction.
#[inline]
pub fn system_call_gas_limit(cfg: impl Cfg) -> u64 {
    SYSTEM_CALL_REGULAR_GAS_LIMIT.saturating_add(system_call_state_gas_reservoir(cfg))
}

/// Creates the system transaction with default values and set data and tx call target to system
/// contract address that is going to be called.
///
/// The caller is set to be [`SYSTEM_ADDRESS`].
///
/// It is used inside [`SystemCallEvm`] and [`SystemCallCommitEvm`] traits to prepare EVM for system
/// call execution.
pub trait SystemCallTx: Sized {
    ...
    /// Sets the system call gas limit.
    fn set_system_call_gas_limit(&mut self, _gas_limit: u64) {}
}
...
impl SystemCallTx for TxEnv {
    fn new_system_tx_with_caller(
        ...
        TxEnv::builder()
            ...
            .gas_limit(SYSTEM_CALL_REGULAR_GAS_LIMIT)
            .build()
            .unwrap()
    }

    fn set_system_call_gas_limit(&mut self, gas_limit: u64) {
        self.gas_limit = gas_limit;
    }
}

/// API for executing the system calls. System calls dont deduct the caller or reward the
/// beneficiary. They are used before and after block execution to insert or obtain blockchain
/// state.
///
/// It act similar to `transact` function and sets default Tx with data and system contract as a
/// target.
///
/// # Note
///
/// Only one function needs implementation [`SystemCallEvm::system_call_one_with_caller`], other
...
pub trait SystemCallEvm: ExecuteEvm {
    ...
    /// Internally calls [`SystemCallEvm::system_call_one`] and [`ExecuteEvm::finalize`] functions
    /// to obtain the changed state.
...
/// Extension of the [`SystemCallEvm`] trait that adds a method that commits the state after
/// execution.
    ...
    PRECOMPILES: PrecompileProvider<CTX, Output = InterpreterResult>,
{
    fn system_call_one_with_caller(
        ...
        // set tx fields.
        let mut tx = CTX::Tx::new_system_tx_with_caller(caller, system_contract_address, data);
        tx.set_system_call_gas_limit(system_call_gas_limit(self.ctx().cfg()));
        self.set_tx(tx);
        // create handler
...
mod tests {
    ...
    use primitives::{b256, bytes, hardfork::SpecId, StorageKey, U256};
    ...
    #[test]
    fn test_amsterdam_system_call_uses_state_gas_reservoir() {
        let mut db = InMemoryDB::default();
        db.insert_account_info(
            HISTORY_STORAGE_ADDRESS,
            AccountInfo::default().with_code(Bytecode::new_legacy(HISTORY_STORAGE_CODE.clone())),
        ...
        let mut evm = Context::mainnet()
            ...
            .modify_block_chained(|b| b.number = U256::ONE)
            .build_mainnet();
        let expected_reservoir = system_call_state_gas_reservoir(evm.ctx.cfg());

        let output = evm.system_call(HISTORY_STORAGE_ADDRESS, block_hash.0.into()).unwrap();

        assert_eq!(evm.ctx.tx().gas_limit(), SYSTEM_CALL_REGULAR_GAS_LIMIT + expected_reservoir);
