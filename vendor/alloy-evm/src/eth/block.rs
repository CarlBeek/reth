    ...
    <R::Transaction as TransactionEnvelope>::TxType: Send + 'static,
{
    ...
    fn execute_transaction_without_commit(
        ...
        if self.evm.cfg_env().enable_amsterdam_eip8037 {
            let regular_available_gas = block_gas_limit.saturating_sub(self.block_regular_gas_used);
            let state_available_gas = block_gas_limit.saturating_sub(self.block_state_gas_used);

            let regular_tx_gas_usage = self
                .evm
                .cfg_env()
                .tx_gas_limit_cap
                .map_or(tx_gas_limit, |cap| min(tx_gas_limit, cap));

            if regular_tx_gas_usage > regular_available_gas {
            ...
            let block_available_gas = block_gas_limit.saturating_sub(self.cumulative_tx_gas_used);
            let max_tx_gas_usage = self
                .evm
                .cfg_env()
                .tx_gas_limit_cap
                .map_or(tx_gas_limit, |cap| min(tx_gas_limit, cap));
