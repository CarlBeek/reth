...
/// - v10: added the durable `ClickHouse` export outbox. Two new tables — `analysis_manifests`
///   (immutable per-`analysis_config_hash` dataset descriptor) and `export_outbox` (one durable row
///   per export-enabled block output, drained by the embedded export worker). The six analytical
///   tables are unchanged, so a v9 DB is not in-place migratable to v10; an export-enabled
///   deployment replays into a fresh database (see `export/model.rs`).
...
///
/// A freshly initialised DB has zero rows in `analysis_runs` /
/// `analysis_manifests` and passes trivially; the version is stamped the first
/// time [`DivergenceDatabase::record_analysis_run_start`] or
/// [`DivergenceDatabase::upsert_analysis_manifest`] is called.
///
/// Both sources are checked: an export-only deployment may write an
/// `analysis_manifests` row before any `analysis_runs` row exists, and once an
/// export-enabled v10 manifest exists a binary compiled for a different schema
/// must fail closed. A v9 DB (with an explicit v9 `analysis_runs` row) is
/// rejected here and requires a fresh replay — there is no in-place migration.
