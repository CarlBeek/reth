...
fn parse_non_zero_usize(value: &str) -> Result<usize, String> {
    let parsed: usize = value.parse().map_err(|err| format!("invalid usize: {err}"))?;
    if parsed == 0 {
...
fn parse_non_zero_u64(value: &str) -> Result<u64, String> {
    let parsed: u64 = value.parse().map_err(|err| format!("invalid u64: {err}"))?;
    if parsed == 0 {
        return Err("value must be greater than 0".to_string());
    }
    Ok(parsed)
...
struct ExportArgs {
    ...
    #[arg(long, default_value_t = 50_000, value_parser = parse_non_zero_usize)]
    row_group_size: usize,
    /// Number of blocks per partition bucket
    #[arg(long, default_value_t = 100_000, value_parser = parse_non_zero_u64)]
